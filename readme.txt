...
* New (off by default): "Oversize Pricing" — parcels with a side of 100 cm or more may be priced by Econt on its cargo tariff, which the standard quote cannot see (measured: a 0.7 kg parcel costs the same in a 20×15×5 and a 90×90×90 cm box, while from a length of 100 cm the price grows with the size — up to several times more). When enabled, the checkout price for such parcels is corrected by the size effect Econt's label API reports for the product dimensions; who pays the delivery is preserved. Parcels under 100 cm are never touched, and if Econt does not answer the price stays as before. Requires the sender address or office in the settings.
* New: the order screen marks parcels with a side of 100 cm or more, which Econt may charge on its cargo tariff, so the delivery cost can be checked before handing the parcel over. It stays quiet when "Oversize Pricing" already priced exactly those items at checkout, and shows whenever that did not happen — option off, the shop pays delivery, Econt did not answer, or the order was entered or changed in the admin. When the option is off it also points to it.
