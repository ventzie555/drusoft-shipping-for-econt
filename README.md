...
> - `get_econt_clients()` still calls a Speedy-shaped `api.econt.bg/v1/client/contract` endpoint. Replace with `ProfileService.getClientProfiles.json`, or remove entirely and rely on the new `econt_store_id` text field.
